...
import { collection, doc, updateDoc, arrayUnion, arrayRemove, orderBy, query, serverTimestamp, onSnapshot }
  from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
import { typeIcon, urgencyBadge, statusBadge, formatTime, showToast, emptyRow, esc, renderPagination, exportToCSV, logAdminAction } from "./utils.js";
...
export function renderReports() {
  ...
  if (paginated.length === 0) {
    ...
    renderPagination('reports-pagination', currentPage, totalPages, 'reportsPrevPage()', 'reportsNextPage()');
    return;
  }
...
window.exportReports = () => {
  const headers = ['Report ID', 'Type', 'Description', 'Urgency', 'Status', 'Submitted By', 'Submitted At', 'Assigned Volunteers', 'Resolution Note', 'Resolved At'];
  const rows = allReports.map(r => [
    ...
    r.status || '',
    r.submittedBy || '',
    r.timestamp?.toDate ? r.timestamp.toDate().toISOString() : r.timestamp || '',
    (r.assignedVolunteers || []).join('; '),
    r.resolutionNote || '',
    r.resolvedAt?.toDate ? r.resolvedAt.toDate().toISOString() : r.resolvedAt || ''
  ]);
  exportToCSV(`reliefnet_reports_${new Date().toISOString().split('T')[0]}.csv`, headers, rows);
};
...
function renderModalContentOnly(r) {
    ...
    : assigned.map(uid => {
        ...
        return `
          <div class="assigned-vol-chip">
            <button class="volunteer-link" onclick="window.openVolunteerProfile('${uid}')" style="color:var(--blue);font-weight:600;">👤 ${name}</button>
  ...
  const dropdownHtml = available.length === 0
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        ...
        <div class="modal-section" style="border-color:var(--green);background:var(--green-light)">
          <div class="modal-section-title" style="color:var(--green)">✅ Resolution</div>
        ...
        <!-- Media Previews -->
        <div class="modal-section">
          <div class="modal-section-title">📎 Media Previews</div>
          ${r.mediaUrls && r.mediaUrls.length > 0
            ? `<div class="media-previews-container">
                ${r.mediaUrls.map(url => {
                  const escapedUrl = esc(url);
                  const isImage = /\.(jpg|jpeg|png|webp|gif)($|\?)/i.test(url) || url.includes("alt=media") && !url.includes("video");
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        ...
        <div class="modal-section" style="background:var(--blue-light);border-color:var(--blue)">
          <div class="modal-section-title" style="color:var(--blue)">✦ AI Analysis <span style="font-size:10px;font-weight:400;opacity:0.7">Powered by Gemini</span></div>
          ...
          <div style="display:flex;flex-direction:column;gap:8px">
            ${r.status !== 'completed' ? `
              <div class="resolve-form">
                ...
                <button class="action-btn btn-resolve" id="resolve-btn" onclick="resolveReport('${r.id}')" style="width:100%;padding:10px;font-size:13px">
                  ✅ Mark as Resolved
                </button>
              </div>
            ` : `<div style="font-size:13px;color:var(--green);font-weight:600">✅ This report has been resolved</div>`}
          </div>
        </div>
...
window.assignVolunteer = async () => {
  ...
  if (!volId) { showToast('⚠️ Select a volunteer first'); return; }
  try {
...
window.removeVolunteer = async (volId) => {
  try {
    ...
    const r = allReports.find(x => x.id === activeReportId);
    if (r) r.assignedVolunteers = (r.assignedVolunteers || []).filter(v => v !== volId);
...
window.resolveReport = async (id) => {
  if (!confirm('Mark this report as resolved? This action cannot be undone.')) return;
  ...
  try {
    ...
    const r = allReports.find(x => x.id === id);
    if (r) { r.status = 'completed'; if (note) r.resolutionNote = note; }
    renderReports();
    updateReportsBadge();
